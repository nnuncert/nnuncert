# Copyright (c) 2016 Hideaki Shimazaki
...
# source code: https://github.com/cooperlab/AdaptiveKDE/blob/master/adaptivekde/ssvkernel.py

# The following minor adaptions have been made to adjust for Python version 3:
# lines (original file) -> adaption
#      79,  88 -> cast to int
#     163, 164 -> cast to int
...
def ssvkernel(x, tin=None, M=80, nbs=1e2, WinFunc='Boxcar'):
    ...
    if tin is None:
        ...
        num_pts = int(min(np.ceil(T / dt_samp), 1e3))
        tin = np.linspace(np.min(x), np.max(x), num_pts)
        t = tin
        ...
        if dt_samp > min(np.diff(tin)):
            num_pts = int(min(np.ceil(T / dt_samp), 1e3))
            t = np.linspace(min(tin), max(tin), num_pts)
        else:
            t = tin
